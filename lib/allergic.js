...
function navigate(obj, trail) {
    ...
    Array.isArray(trail) || (trail = trail.split('.'));
...
module.exports = function (dust, options) {
	...
	dust.helpers.react = function (chunk, context, bodies, param) {
		...
		return chunk.map(function(chunk) {
			...
			// component loading
			loadComponent(relativePath, param.component, function(err, Component) {
				console.log('loadComponent complete');
				if (err) {
					console.error(err.stack);
					...
					mapContent(bundle, locality, param, enableMetadata, function(err, mappedParams) {
						console.log('mapContent complete');
						var reactString;
						...
						try {
							reactString = componentWrap(React.renderToString(Component(mappedParams)), componentId);
						} catch(renderError) {
