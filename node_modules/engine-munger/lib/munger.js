...
exports.wrapDustOnLoad = function (ext, config, needCache, app) {
    console.log('engine-munger:lib/munger:wrapDustOnLoad(\n\text = %s,\n\tconfig = %s,\n\tneedCache = %s,\n\tapp = %s)',ext,JSON.stringify(config,null,4),needCache,app);

    var conf = {},
        viewCache,
        i18n = config.i18n;

    // BEN: all this does is just multiplex between a "normal" loader or a loader that comes from the views/ folder.
    ...
    dustjs.onLoad = function spclOnLoad(name, context, cb) {
        console.log('\n-------------- START lib/munger.js (%s) -------------------------\n',name);
        console.log('engine-munger:lib/munger:wrapDustOnLoad.onLoad(\n\tname = %s,\n\tcontext = %s,\n\tcb = %s)',name,'<context>','<callback>');
        ...
        console.log('engine-munger:lib/munger:wrapDustOnLoad.onLoad:specialization = %s, mappedName = %s', specialization, mappedName);

        // This should belong to the function loaded from the view.
        onLoad(mappedName, context, function (err, data) {
            ...
            console.log('engine-munger:lib/munger:wrapDustOnLoad.onLoad.onLoad ????? data = %s', data);
            console.log('\n------------ END lib/munger.js (%s) ---------------------------\n',name);
            cb(null, data);
        });
    };
